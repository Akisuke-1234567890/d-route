...
- Route参加者（管理者を含む）にMemberと共通の6色の識別色を追加。
- Members画面から自分の識別色を選択・変更可能。
- ChatではAアイコンを廃止し、参加者名を識別色で表示。
- Chatの自分側・他参加者側とも同じMemberカラー体系を利用。
- 既存の招待・参加状態・Chat本文・RC5のタブレット表示修正を維持。

# 2.1.1-RC5

- タブレット等のWeb表示で、自分の短いChat本文が1文字ずつ縦に折り返される問題を修正。
- Chatの自分側メッセージ行に明示的な幅を持たせ、短文でも吹き出しが極端に縮まないよう調整。
- スマホ側のChat表示・既存の受信メッセージ表示は維持。
- 2.1.1-RC4のRoute参加者Dashboard変更は維持。

# 2.1.1-RC4

- 参加者のRoute Dashboardを管理者と同じPhase / 予定カード構成で表示。
- 「いま必要な流れ」の専用表示を廃止。
- 次のPhaseを確認できるPhaseナビを参加者にも表示。
- Placesの管理リンクは参加者には表示しない。
- 予定の完了操作は参加者から変更できないよう維持。
- 2.1.1-RC3の招待Route表示・Routeオープン処理は維持。

# 2.1.1-RC3

- 招待Routeが一覧から消える回帰を修正。
- Route一覧取得を `list_my_routes` RPC に戻し、未回答/参加中メンバーを取得対象に復元。
- RC2のRoute詳細取得・参加後の読み取り修正は維持。

# 2.1.1-RC2

- 招待されたユーザーがRoute本体を開けない問題を修正。
- `get_my_route` RPCで未回答招待でもRoute情報を取得可能に変更。
- 参加後のPhase / Placesを参加者が読み取れるようRLSを修正。
- 2.1.0の画面遷移・下帯・Main/Sub Route演出は変更なし。

# 2.1.0

- RC2.4 を正式版 2.1.0 として固定。
- PWA / Apple Touch Icon のキャッシュ識別子を 2.1.0 に統一。
- RC2.4 の画面遷移・下帯・主要機能ロジックを維持。
- DB変更なし。
...
- Route / Places / Chat / Members / Menu の機能ロジックは変更なし
- RC2.2の画面遷移・下帯挙動を維持
...
- Route / Places は裏描画のため少し長め、Chat / Members / Menu は短めのテンポに調整。
- 通常のタブ切替で「画面を準備しています」ロード画面を表示しないよう変更。
- Main/Sub Route の画面内切替は既存挙動を維持。

# 2.1.0-RC1.5

- RC最終整理: 本番に不要なiPhone API / Updaterテストファイルを削除。
- PlacesのサブRoute保存処理に残っていた開発用console.errorを削除し、ユーザー向け警告動作は維持。
- CHANGELOG内で崩れていたRC1.1 / RC1.2の見出しを正常化。
