...
export function AccountSetupPage({ user, onCompleted }: { user: User; onCompleted: () => Promise<void> }) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    setLoading(true);
    try {
      await completeAccountSetup({ loginId, password, passwordConfirm, displayName: legacy ? undefined : displayName });
      await onCompleted();
  ...
  return <main className="auth-page auth-preview-page"><section className="auth-card auth-preview-card" aria-labelledby="setup-title">
    ...
    <form onSubmit={handleSubmit} noValidate>
      ...
      {!legacy && <><label htmlFor="setup-display-name">表示名</label><input id="setup-display-name" autoComplete="nickname" value={displayName} onChange={(event)=>setDisplayName(event.target.value)} placeholder="たけだ" /></>}
